...
Pixelizer is a small utility to create pixelated (retro video game style) images and videos.

Pixelizer applies the following steps in order:

1. Resizes the image (default is 256px wide) while preserving aspect ratio.
2. Adjusts contrast and exposure.
3. Maps image colors to a chosen palette.
4. Applies dithering (Floyd–Steinberg, Sierra, or Bayer) to blend colors.
5. Scales up the image with nearest-neighbor so pixels remain crisp.

The palettes were sourced from [Lospec's palette gallery](https://lospec.com/palette-list).

## Install

Create a virtual environment and install the project locally so that the `pixelizer` CLI becomes available:
...
After installation you can run `pixelizer` with one of four subcommands:
...
- `pixelizer live` — interactive webcam preview (requires OpenCV)
...
- `-p, --palette` Palette name to use (default: None; falling back to the built-in default palette)
- `-pf, --palette-file` YAML file containing palettes (name -> list of hex colors). Keys are lowercased when loaded.
...
**Heads Up**: This can take several minutes depending on the number of palettes in your `palettes.yml`.

`--palette` and `--dither` are ignored — the sample command iterates all palettes found in the provided `--palette-file` and the internal `SAMPLE_DITHERS` list.
...
```bash
pixelizer live -o out.mp4 -w 128 -s 8
```

Live will show a feed from your webcam. The `up` and `down` arrow keys will change the dithering level. The `left` and `right` arrows change the palette. `spacebar` will snap a photo with the current settings.

## Examples

The original example images are included in the repo's examples and are generated by the CLI. Output filenames follow this pattern when not provided: `{input_name}_{palette}_{dither}_{width}px_{scale}x.{ext}`. For `sample` outputs the dither field is omitted and the palette is `sample`.

Live mode requires `opencv-python` and will only import it when running `pixelizer live`, so day-to-day usage of other subcommands does not require OpenCV.
